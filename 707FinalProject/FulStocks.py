...
yf.pdr_override()
stock_data = pdr.get_data_yahoo(["^GSPC"]+stocks, start="2016-01-01", end="2021-01-01")['Adj Close']
data = t.tensor(stock_data.values).float().T
Loss = sMAPE
...
img_tile(plots_array_forecast[0:10], warmstart_iterations, 'DJI Subset Forecast Trend Warm Start')
...
while diff > 1e-6:
    ...
    if iterations % 1000 == 0:
        with t.no_grad():
            ...
            img_tile(plots_array_forecast[0:10], iterations, "DJI Subset validation_trend")
            plots_array_seasonal = make_plots(nbeats.seasonal_predictions.detach().numpy())
            img_tile(plots_array_seasonal[0:10], iterations, "DJI Subset validation_seasonal")
            plots_array_seasonal = make_plots(f.detach().numpy(),data[:,-20:].detach().numpy())
            img_tile(plots_array_seasonal[0:10], iterations, "DJI_Subset validation_full")


print(iterations)
plt.plot(loss_values)
plt.title('Loss at every 100th iteration')
plt.show()
